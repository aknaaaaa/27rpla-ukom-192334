    ...
    <div class="profile-hero">
            ...
            <div class="profile-panel">
                @if(($tab ?? 'profile') === 'history')
                    ...
                    <!-- Tab Navigation -->
                    <div style="display: flex; gap: 12px; margin-bottom: 16px; border-bottom: 2px solid #e0d4c7;">
                        <a href="{{ route('profile.profile', ['tab' => 'history', 'sub' => 'ongoing']) }}" 
                           style="padding: 12px 16px; text-decoration: none; color: {{ ($subTab ?? 'ongoing') === 'ongoing' ? '#7c6044' : '#8a7c70' }}; border-bottom: 3px solid {{ ($subTab ?? 'ongoing') === 'ongoing' ? '#7c6044' : 'transparent' }}; font-weight: {{ ($subTab ?? 'ongoing') === 'ongoing' ? '700' : '600' }}; margin-bottom: -2px; transition: all 0.2s ease;">
                            Sedang Berlangsung
                        </a>
                        <a href="{{ route('profile.profile', ['tab' => 'history', 'sub' => 'completed']) }}" 
                           style="padding: 12px 16px; text-decoration: none; color: {{ ($subTab ?? 'ongoing') === 'completed' ? '#7c6044' : '#8a7c70' }}; border-bottom: 3px solid {{ ($subTab ?? 'ongoing') === 'completed' ? '#7c6044' : 'transparent' }}; font-weight: {{ ($subTab ?? 'ongoing') === 'completed' ? '700' : '600' }}; margin-bottom: -2px; transition: all 0.2s ease;">
                            Sudah Selesai
                        </a>
                    </div>

                    <!-- Ongoing Orders -->
                    @if(($subTab ?? 'ongoing') === 'ongoing')
                        @forelse(($ongoingOrders ?? []) as $order)
                            <div class="booking-item" style="margin-bottom: 10px; border-left: 4px solid #667eea;">
                                <strong style="color: #7c6044;">{{ $order->kamar->nama_kamar ?? 'Kamar' }}</strong>
                                <p style="font-size: 12px; color: #8a7c70; margin: 6px 0;">
                                    <i style="color: #667eea;">📅</i> Check-in: {{ optional($order->tanggal_checkin)->format('d M Y') }} 
                                    <br><i style="color: #667eea;">📤</i> Check-out: {{ optional($order->tanggal_checkout)->format('d M Y') }}
                                </p>
                                <p style="font-size: 13px; color: #7c6044; margin: 6px 0;">
                                    <strong>Kode Booking:</strong> {{ $order->kode_pesanan }}
                                </p>
                                <p style="font-size: 13px; color: #28a745; margin: 6px 0;">
                                    ✓ Status Pembayaran: {{ strtoupper($order->pembayaran->status_pembayaran ?? 'Belum dibayar') }}
                                </p>
                                <p style="font-size: 13px; color: #7c6044; margin: 6px 0;">
                                    Rp{{ number_format($order->pembayaran->amount_paid ?? 0, 0, ',', '.') }}
                                </p>
                            </div>
                        @empty
                            <div class="booking-item">
                                <strong style="color: #8a7c70;">Tidak ada pemesanan yang sedang berlangsung</strong>
                    ...
                    @else
                        ...
                        @forelse(($completedOrders ?? []) as $order)
                            <div class="booking-item" style="margin-bottom: 10px; border-left: 4px solid #28a745;">
                                <strong style="color: #7c6044;">{{ $order->kamar->nama_kamar ?? 'Kamar' }}</strong>
                                <p style="font-size: 12px; color: #8a7c70; margin: 6px 0;">
                                    <i style="color: #28a745;">📅</i> Check-in: {{ optional($order->tanggal_checkin)->format('d M Y') }} 
                                    <br><i style="color: #28a745;">📤</i> Check-out: {{ optional($order->tanggal_checkout)->format('d M Y') }}
                                </p>
                                <p style="font-size: 13px; color: #7c6044; margin: 6px 0;">
                                    <strong>Kode Booking:</strong> {{ $order->kode_pesanan }}
                                </p>
                        ...
                        @empty
                            <div class="booking-item">
                                <strong style="color: #8a7c70;">Belum ada pemesanan yang selesai</strong>
                                <p style="font-size: 12px; color: #8a7c70;">Pemesanan Anda akan muncul di sini setelah masa menginap selesai.</p>
                            </div>
                        @endforelse
                    @endif
                            ...
                            @forelse(($orders ?? []) as $order)
                                <div class="booking-item">
                                    ...
                                    <p style="font-size: 12px; color: #8a7c70;">
                                        Check-in {{ optional($order->tanggal_checkin)->format('d M Y') }}
                                    </p>
                                    @php
